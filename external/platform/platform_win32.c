...
bool platform_create_window(platform_state_t *state, platform_window_t **window, const platform_window_desc_t *desc) {
    ...
    *window = malloc(sizeof(platform_window_t));
    if (!*window) {
        return false;
    }
    ...
    (*window)->hwnd = CreateWindowExW(
        ...
        x, y,
        width, height,
        desc->parent ? desc->parent->hwnd : NULL,
        NULL,
...
void *platform_window_get_native_handle(platform_window_t *window) {
    assert(window);
    return window->hwnd;
}

struct int2 platform_window_get_resolution(platform_window_t *window) {
    return (int2_t){window->width, window->height};
}
