...
VriResult d3d11_device_create(const VriDeviceDesc *p_desc, VriDevice *p_device) {
    ...
    *p_device = vri_object_allocate(*p_device, &p_desc->allocation_callback, device_size, VRI_OBJECT_DEVICE);
    ...
    // In that case we can use NULL for the adapter and D3D_DRIVER_TYPE_HARDWARE
    {
        ...
        if (FAILED(hr)) {
            dbg.pfn_message_callback(VRI_MESSAGE_SEVERITY_FATAL, "Couldn't get IDXGIAdapter");
    ...
    if (FAILED(hr)) {
        dbg.pfn_message_callback(VRI_MESSAGE_SEVERITY_FATAL, "Failed to create D3D11 device with any driver type");
    ...
    if (FAILED(hr)) {
        dbg.pfn_message_callback(VRI_MESSAGE_SEVERITY_FATAL, "Couldn't upgrade to ID3D11Device5. Feature not supported");
        p_desc->allocation_callback.pfn_free(*p_device, device_size, 8);
    ...
    if (FAILED(hr)) {
        dbg.pfn_message_callback(VRI_MESSAGE_SEVERITY_FATAL, "Couldn't upgrade to ID3D11DeviceContext4. Feature not supported");
        p_desc->allocation_callback.pfn_free(*p_device, device_size, 8);
    ...
    if (p_desc->enable_api_validation) {
        ...
        if (SUCCEEDED(hr) && info_queue) {
            ...
            dbg.pfn_message_callback(VRI_MESSAGE_SEVERITY_ERROR, "Failed to enable D3D11 debug layer");
        }
    }
